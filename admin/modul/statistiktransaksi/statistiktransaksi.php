...
<form id="contactform" class="rounded" action="?modul=statistiktransaksi&aksi=cetak" method="post" enctype="multipart/form-data">';
echo '<div id="papanpanel2">
	<h3>Cetak Data Statistik Berdasarkan Tanggal Transaksi</h3>
	</div>  ';
?>
...
echo "</br></br><b>Berikut Adalah Statistik Jumlah Transaksi Dari \"$_POST[dari]\" Sampai \"$_POST[sampai]\"</b>";
}else{
echo "</br></br><b>Berikut Adalah Statistik Jumlah Transaksi Dari \"$kurangwaktu\" Sampai \"$tglsekarang\"</b>";
}

		if ((isset($_POST['dari'])) && (isset($_POST['sampai']))){
		 if ($_SESSION['levelmembership']=="member") {
		 $tmpid = mysqli_fetch_array(mysqli_query($conn,"select id_member from tbl_member where user = '$_SESSION[usermembership]'"));
		 $query = mysqli_query($conn,"SELECT tanggal_transaksi, count(tanggal_transaksi) as total FROM tbl_trans_deposit where (tanggal_transaksi between '$_POST[dari]' and '$_POST[sampai]') and id_member = '$tmpid[id_member]' GROUP BY tanggal_transaksi ORDER BY tanggal_transaksi ASC");
		 $query2 = mysqli_query($conn,"SELECT tanggal_transaksi, count(tanggal_transaksi) as total FROM tbl_trans_deposit where (tanggal_transaksi between '$_POST[dari]' and '$_POST[sampai]') and id_member = '$tmpid[id_member]' GROUP BY tanggal_transaksi ORDER BY tanggal_transaksi ASC");
		 }else{
		$query = mysqli_query($conn,"SELECT tanggal_transaksi, count(tanggal_transaksi) as total FROM tbl_trans_deposit where tanggal_transaksi between '$_POST[dari]' and '$_POST[sampai]' GROUP BY tanggal_transaksi ORDER BY tanggal_transaksi ASC");
		$query2 = mysqli_query($conn,"SELECT tanggal_transaksi, count(tanggal_transaksi) as total FROM tbl_trans_deposit where tanggal_transaksi between '$_POST[dari]' and '$_POST[sampai]' GROUP BY tanggal_transaksi ORDER BY tanggal_transaksi ASC");
		}
		}
?>
